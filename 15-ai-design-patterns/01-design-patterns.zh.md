...
本章彙整了建構 AI 系統的常見模式，類似於軟體工程中的設計模式。每個模式都包含適用情境、實作指引與權衡取捨。
...
### 模式：Corrective RAG（CRAG，校正型 RAG）
...
實作請參閱 [Agent 架構](../07-agentic-systems/01-agent-architectures.md)。
...
### 模式：Hierarchical Agents（階層式 agent）

由管理者將工作委派給專責的 worker：
...
- 重複出現的相似查詢
- 以降低成本為優先
- 可以容忍一定程度的資料過期（staleness）

---

## 可靠性模式

### 模式：Retry with Fallback（重試加備援）
...
### Q：描述三種 RAG 模式，以及各自的適用時機。

**理想回答：**

「我會描述 Naive RAG、Advanced RAG 與 Parent-Child Retrieval。

**Naive RAG** 是最簡單的：嵌入查詢、搜尋向量、把 top K 塞進 prompt、生成。我會在 MVP 階段，以及檢索品質本來就不錯時使用它。它實作起來很快，但沒有 reranking 或查詢增強。

**Advanced RAG** 加入了多個階段：查詢改寫（query rewriting）、hybrid search（語意 + 關鍵字）、reranking 與過濾。當準確度很重要時，我會在正式環境使用它。為了換取 10-15% 的精準度提升，多出來的延遲（reranking 約 100-200ms）是值得的。

**Parent-Child Retrieval** 嵌入小型 chunk 以進行精準比對，但回傳較大的父層 chunk 以提供上下文。當文件具有結構，而我同時需要檢索的精準度與生成所需的足夠上下文時，我會使用它。

我選擇哪種模式，取決於準確度需求、延遲預算與文件特性。我通常會先用 Naive RAG 建立基準，再迭代到 Advanced RAG。」

### Q：對於正式環境的 LLM 系統，你會使用哪些可靠性模式？

**理想回答：**

「我會實作多層次的可靠性：

**Retry with exponential backoff（指數退避重試）** 用於處理短暫性故障。LLM API 常見 rate limit 與暫時性錯誤。

**Multi-provider fallback（多供應商備援）**，這樣一來，若 OpenAI 出問題，我會自動路由到 Anthropic 或 Google。這需要對 LLM 介面進行抽象化。

**Circuit breaker（斷路器）** 用於停止持續轟炸故障中的服務。在 N 次失敗後，我會打開斷路器並立即路由到備援，給主要服務時間復原。

**Graceful degradation（優雅降級）** 用於所有供應商都故障時。回傳快取的回應、顯示備援訊息，或排入佇列稍後處理，而不是直接報錯。

**Bulkhead isolation（艙壁隔離）** 用於防止某個元件的故障擴散。Agent 工作負載與 RAG 工作負載使用各自獨立的 thread pool。

**Timeouts（逾時）** 設置在每一個層級。LLM 呼叫可能會卡住；我會設定積極的逾時時間並優雅地處理它們。

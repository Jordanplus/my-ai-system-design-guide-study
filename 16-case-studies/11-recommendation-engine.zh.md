# 案例研究：AI 驅動的推薦引擎

## 問題描述

一個擁有 **5,000 萬使用者** 的串流平台需要建立一套推薦系統，將協同過濾（collaborative filtering）與 LLM 產生的解釋結合在一起：「因為你喜歡《全面啟動》（Inception），你可能會喜歡《天能》（Tenet），它有著燒腦的時間機制。」

**面試中給定的限制條件：**
- 即時推薦（p95 低於 200ms）
- 必須解釋為何做出每一項推薦
- 處理新使用者的冷啟動（cold-start）
- 隱私：不可在使用者之間洩漏觀看紀錄
- 每日活躍：500 萬使用者，每人觀看 10 組以上的推薦結果

---

## 面試題目

> 「設計一套系統，大規模地推薦電影，並以自然語言解釋該推薦的理由。」

---

## 解決方案架構

```mermaid
flowchart TB
    subgraph Offline["Offline Pipeline (Daily)"]
        HISTORY[(Watch History)] --> EMBED[User Embedding<br/>Matrix Factorization]
        CATALOG[(Content Catalog)] --> CONTENT_EMBED[Content Embeddings]
        EMBED --> CANDIDATES[Candidate Generation<br/>ANN Index]
    end

    subgraph Online["Online Serving (Real-Time)"]
        USER[User Request] --> FETCH[Fetch User Embedding]
        FETCH --> ANN[ANN Search<br/>Top 100 Candidates]
        ANN --> RERANK[Reranker<br/>Cross-Encoder]
        RERANK --> TOP10[Top 10 Results]
    end

    subgraph Explain["Explanation Generation"]
        TOP10 --> BATCH[Batch Explanation Request]
        BATCH --> LLM[GPT-4o-mini<br/>Cached Explanations]
        LLM --> RESPONSE[Recommendations + Reasons]
    end
```

---

## 關鍵設計決策

### 1. 為什麼不直接全部用 LLM 來做？

**答案：** 規模的經濟性。為 5,000 萬使用者 × 每日 10 組推薦呼叫 LLM = 每日 5 億次 LLM 呼叫。以每次呼叫 $0.001 計算，那就是每日 $500K。取而代之的做法是：

| 元件 | 角色 | 每位使用者每日成本 |
|-----------|------|-------------------|
| Embedding 查找 | 取出預先計算好的向量 | $0.00001 |
| ANN 搜尋 | 找出候選項 | $0.0001 |
| Cross-encoder 重排序 | 為前 100 名評分 | $0.001 |
| LLM 解釋 | 自然語言 | $0.005 |
| **總計** | | **$0.006** |

LLM 僅用於產生最終的解釋，而非排序本身。

### 2. 解釋快取

**答案：** 大多數解釋都可以被快取。「因為你看過《全面啟動》」這句話適用於數以千計的使用者。我們在 (content_pair, reason_type) 這個層級快取解釋：

```python
cache_key = f"{source_movie}:{target_movie}:{reason_type}"
# Example: "inception:tenet:time_mechanics"

explanation = cache.get(cache_key)
if not explanation:
    explanation = generate_explanation(source_movie, target_movie, reason_type)
    cache.set(cache_key, explanation, ttl=86400)
```

快取命中率：暖機後達 85%+。

### 3. 冷啟動處理

**答案：** 新使用者沒有可供協同過濾使用的歷史紀錄。我們採用 **混合式做法（Hybrid Approach）**：

```mermaid
flowchart LR
    NEW_USER[New User] --> CHECK{Has History?}
    CHECK -->|No| CONTENT[Content-Based<br/>Preferences Survey]
    CHECK -->|Yes, <10 items| HYBRID[Hybrid:<br/>Content + Collaborative]
    CHECK -->|Yes, >10 items| COLLAB[Full Collaborative<br/>Filtering]
    
    CONTENT --> RECS[Recommendations]
    HYBRID --> RECS
    COLLAB --> RECS
```

---

## 個人化解釋的挑戰

解釋必須讓人感覺是為個人量身打造，而非千篇一律：

**不好的範例：**「《天能》是一部熱門驚悚片。」
**好的範例：**「因為你喜歡《全面啟動》燒腦的劇情，《天能》提供了出自同一位導演、類似的時間操弄謎題。」

我們透過在 prompt 中納入使用者情境來達成這一點：

```python
prompt = f"""
Generate a 1-sentence explanation for why this user would enjoy {target_movie}.

User context:
- Recently watched: {recent_movies}
- Preferred genres: {genres}
- Dislikes: {dislikes}

Source movie that triggered this recommendation: {source_movie}
Reason category: {reason_type}

Explanation:
"""
```

---

## 延遲預算

| 階段 | 目標 | 實際 p95 |
|-------|--------|------------|
| 使用者 embedding 查找 | 5ms | 3ms |
| ANN 搜尋（前 100 名） | 20ms | 15ms |
| Cross-encoder 重排序 | 50ms | 45ms |
| LLM 解釋（快取命中） | 10ms | 8ms |
| LLM 解釋（未命中） | 500ms | 450ms |
| **總計（快取命中）** | **85ms** | **71ms** |
| **總計（快取未命中）** | **575ms** | **513ms** |

為了達到 200ms p95，我們確保解釋的快取命中率達 95%+，並針對新的內容配對非同步地產生解釋。

---

## 面試追問

**Q：你如何防止 LLM 對電影捏造（hallucinate）事實？**

A：LLM 會收到每部電影的結構化事實表（fact sheet）作為情境（導演、卡司、主題、獲獎紀錄）。它只能使用這份表上的資訊。我們也有一個產生後驗證器（post-generation validator），會將其陳述與我們的目錄中介資料（catalog metadata）進行比對。

**Q：如果使用者的喜好快速改變怎麼辦？**

A：我們採用 **近期加權的 embedding 更新（recency-weighted embedding update）**。近期觀看的權重比較舊的高出 3 倍。為了達到即時反應，我們維護一個「session embedding」，用以捕捉當前工作階段（session）的行為，並將其與歷史 embedding 融合。

**Q：你如何對推薦演算法進行 A/B 測試？**

A：我們將 user_id 雜湊（hash）後，將使用者一致地分配到實驗分組（bucket）。每個分組可以採用不同的候選項生成、排序或解釋策略。我們追蹤每個分組的參與度指標（點擊率、觀看時間、跳過率）。

---

## 面試重點整理

1. **LLM 用於解釋，而非排序**：用傳統 ML 來因應規模，用 LLM 來做個人化
2. **積極地快取**：內容配對的解釋可在使用者之間重複使用
3. **冷啟動是一個光譜**：新使用者 → 以內容為基礎；有部分歷史 → 混合式；完整歷史 → 協同過濾
4. **延遲預算需要快取命中率目標**：圍繞著你的延遲 SLA 來設計快取

---

*相關章節：[語意快取](../08-memory-and-state/05-semantic-caching.md)、[成本最佳化](../04-inference-optimization/07-cost-optimization-playbook.md)*

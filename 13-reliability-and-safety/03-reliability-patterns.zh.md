...
生產環境的 LLM 系統需要超越基本重試邏輯的健全可靠性模式。本章涵蓋用於建構具韌性 AI 應用的進階模式。
...
| 失效模式 | 成因 | 影響 |
|--------------|-------|--------|
| Rate limiting | 配額超出 | 請求被拒絕 |
| 逾時 | 長時間生成、網路問題 | 回應緩慢／失敗 |
| 供應商中斷 | 基礎設施問題 | 完全失效 |
| 品質劣化 | 模型更新、負載 | 輸出變差 |
| Context 溢位 | 輸入過大 | 請求失敗 |
| 格式錯誤的輸出 | 生成錯誤 | 解析失敗 |
...
**帶退避的重試：** 針對暫時性失效採用帶抖動的指數退避。重點在於區分可重試錯誤（rate limits、逾時）與不可重試錯誤（驗證、錯誤的請求）。

**斷路器：** 如果某個供應商反覆失敗，就停止嘗試一段冷卻期。這可避免在已死的供應商上浪費延遲，並給它時間恢復。

**多供應商容錯切換：** 絕不依賴單一供應商。我會設定 primary／secondary／tertiary 並自動容錯切換。每個供應商都有自己的斷路器。

**優雅降級：** 定義當沒有任何供應商可用時會發生什麼。回傳降級後的回應（較簡單的模型、快取結果）優於完全失敗。

**艙壁隔離：** 隔離不同的工作負載。批次處理的尖峰不應該拖垮即時查詢。

關鍵洞見在於假設會失敗。LLM API 比傳統 API 更不可靠。要假設供應商會當機來設計，因為它一定會。」
...
**重試**處理暫時性失效。如果單一請求失敗，就再試一次。它假設失效是獨立的，下一次嘗試可能會成功。

**斷路器**處理系統性失效。如果許多請求都在失敗，就完全停止嘗試。它假設下游系統處於不健康狀態，反覆嘗試會浪費資源並拖慢恢復。

**它們如何協同運作：**
1. 請求失敗 → 帶退避的重試（嘗試 1、2、3）
2. 如果所有重試都失敗 → 斷路器記錄一次失效
3. 在 N 次失效後 → 斷路器開啟，立即拒絕請求
4. 在逾時後 → 斷路器半開，允許有限的測試請求
5. 如果測試成功 → 斷路器關閉，恢復正常運作

沒有斷路器時：在中斷期間，每個請求都要走完所有重試才會失敗。延遲飆升、資源耗盡。

有斷路器時：偵測到中斷後，請求會快速失敗。系統保持回應性，並可容錯切換至替代方案。」
...
*上一篇：[Ensemble Methods](02-ensemble-methods.md)*
